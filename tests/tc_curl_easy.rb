...
class TestCurbCurlEasy < Test::Unit::TestCase
  ...
  def test_threads
    ...
    5.times do
      threads << Thread.new do
        5.times do
          curl = Curl.get($TEST_URL)
          assert_match(/^# DO NOT REMOVE THIS COMMENT/, curl.body_str)
          assert_match(/^# DO NOT REMOVE THIS COMMENT/, curl.body)
          assert_equal "", curl.header_str
          assert_equal "", curl.head
        end
      end
    end
  ...
  def test_patch_remote
    ...
    assert curl.http_patch("message")
    assert_match(/^PATCH/, curl.body_str)
  ...
  def test_patch_data_null_bytes
    curl = Curl::Easy.new(TestServlet.url)
    curl.patch_data = "a\0b"
  ...
  def test_patch_nil_data_no_crash
    curl = Curl::Easy.new(TestServlet.url)
    curl.patch_data = nil
  ...
  def test_patch_remote_file
    curl = Curl::Easy.new(TestServlet.url)
    File.open(__FILE__,'rb') do|f|
      assert curl.http_patch(f)
    end
  ...
  def test_patch_class_method
    count = 0
    curl = Curl::Easy.http_patch(TestServlet.url,File.open(__FILE__,'rb')) do|c|
      count += 1
      assert_equal(Curl::Easy, c.class)
    end
